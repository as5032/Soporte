...
function redirecciona($pd, $pm, $xc)
{
    return header("Location: https://google.com/");
}

function actualiza_estatus($postdata, $primary, $xcrud)
{
    date_default_timezone_set('America/Mexico_City');
    $primary = (int)$xcrud->get('primary');
    $db = Xcrud_db::get_instance();
    if ($postdata->get('status') == 2)
    {
      $publish_date =date("Y-m-d H:i:s");
      $db->query('UPDATE tickets set date_closed = \''.$publish_date.'\' WHERE id = ' . $primary . ' LIMIT 1'   );
    }
}
...
{
  $db = Xcrud_db::get_instance();
  $pass =  md5('12345');// md5($postdata->get('password'));
  $db->query("UPDATE users set password = '$pass' LIMIT 1");
  //$postdata->set('password', $pass);
}
...
function alogs($xcrud)
{
    ...
    $sql2 = "UPDATE logs SET logs.updated_by={$_SESSION["login_id"]} WHERE logs.logs_id= $Maxid";
