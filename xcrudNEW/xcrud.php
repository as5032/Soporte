...
class Xcrud
{
    ...
    private $routine_time_in_seconds = 0; 
    ...
    protected $is_bulk_delete = false;
    ...
    protected $column_rotation = ""; 
    protected $rotation_angle = "";
    protected $column_switch = "";
    protected $column_fix = "";
    protected $column_resize = "";
    protected $column_visibility_control = "";
    ...
    public function set_bulk_delete($bool = true)
    {
        $this->is_bulk_delete = (bool)$bool;       
        return $this;
    }
    public function set_bulk_edit($bool = true)
    {
        $this->is_bulk_edit = (bool)$bool;
        return $this;
    }
    ...
    public function button($link = '', $name = '', $icon = '', $class = '', $parameters = array(), $condition = array(), $form = array())
    {
        if ($link)
        {
            $this->buttons[] = array(
    ...
    public function set_routine($action = '',$time_in_seconds = '')
    {
  
        if($this->action_event != ""){
            $this->action_event = $this->action_event . ",$action";
        }else{
            $this->action_event = "$action";
        }
        $this->routine_time_in_seconds = $time_in_seconds; 
        return $this;
    }


    
    public function set_column_visibility_control()
    {

        if($this->action_event != ""){
            $this->action_event = $this->action_event . ",column_visibility_control";
        }else{
            $this->action_event = "column_visibility_control";
        }

        $this->column_visibility_control = true;
        return $this;
    }


    public function set_column_resize($columns = '')
    {

        if($this->action_event != ""){
            $this->action_event = $this->action_event . ",columnresize";
        }else{
            $this->action_event = "columnresize";
        }

        $this->column_resize = $columns;
        return $this;
    }

    public function set_column_rotation($columns = '',$angle = '')
    {
        if($this->action_event != ""){
            $this->action_event = $this->action_event . ",columnrotation";
        }else{
            $this->action_event = "columnrotation";
        }

        
        $this->column_rotation = $columns;
        $this->rotation_angle = $angle; 
        return $this;
    }

    public function set_column_switch()
    {

    
        if($this->action_event != ""){
            $this->action_event = $this->action_event . ",columnswitch";
        }else{
            $this->action_event = "columnswitch";
        }

        $this->column_switch = true;
        return $this;
    }

    public function set_column_fix()
    {
        if($this->action_event != ""){
            $this->action_event = $this->action_event . ",columnfix";
        }else{
            $this->action_event = "columnfix";
        }
        $this->column_fix = true;
        return $this;
    }



    public function set_formular($fields = '',$formular = '', $event = '', $target = '')
    {

        $formular_tmp = $formular;
        //$formular = "{field1}*{field2}";

        // Regular expression to match all strings inside curly braces
        preg_match_all('/\{([^}]*)\}/', $formular, $matches);

        // $matches[1] will contain all the strings inside the curly braces
        $fieldsArray = $matches[1];
        ...
        foreach($fieldsArray as $field_specific){
            
            $uniqueId_specific = uniqid("");
            $formular_tmp = str_replace($field_specific, $uniqueId_specific, $formular_tmp);

            if (in_array($field_specific, $targetArray)) {
                $this->set_attr("$field_specific",array("formuar-name"=>"$uniqueId_specific","formular-identifier"=>"$uniqueId","formular-event"=>"$event"));
            } else {
                $this->set_attr("$field_specific",array("formuar-name"=>"$uniqueId_specific","formular-identifier"=>"$uniqueId"));
            }

            
        }

        $formular_encoded = urlencode($formular_tmp);
        $this->set_attr("$fields",array("formular-answer"=>"$uniqueId","formular"=>"$formular_encoded"));
    ...
    protected function _update($postdata, $primary)
    {
        ...
        if (!$this->join)
        {
            if (!$this->demo_mode)
                ...
                if($this->primary_val_multi){
                    //echo "YES" . $this->primary_val_multi;

                    $array_val = $this->primary_val_multi;
                    $trimmed_array_val = trim($array_val, "'");

                    $res = $db->query("UPDATE `{$this->table}` SET " . implode(",\r\n", $set) . " WHERE `{$this->primary_key}` IN (" . $trimmed_array_val . ")");
                    //echo "UPDATE `{$this->table}` SET " . implode(",\r\n", $set) . " WHERE `{$this->primary_key}` IN (" . $trimmed_array_val . ")";
                }else{
        ...
        else
        {
            ...
            if (!$this->demo_mode)
                ...
                if($this->primary_val_multi){
                    ...
                    " WHERE `{$this->table}`.`{$this->primary_key}` IN (" . $trimmed_array_val . ")");
                }else{
    ...
    protected function _save()
    {
        ...
        $this->primary_val_multi = $this->_post('primaryarray');
    ...
    protected function _build_where()
    {
        ...
        if ($this->where_pri)
        {
            foreach ($this->where_pri as $params)
            {
                ...
                {
                    if(is_array($db->escape($params['value']))){
                        $c = $db->escape($params['value']);
                        $result_string = implode(',', $c);
                        $c_s = count($c);
                        echo "<label>Editing $c_s records</label>";
                        echo "<input type='hidden' name='primaryarray' value='$result_string'>";
                        $where_arr_pri[] = $this->_where_field($params) . $this->_cond_from_where($params['field']) . -1; //$db->escape($params['value'])[0];
                    }else{
    ...
    {
        $this->primary_val_multi = $this->_post('primaryarray');
        //echo "dddd" . $this->primary_val_multi;
        //$this->primary_val_multi = $this->_get('primaryarray');
    ...
    protected function create_button($name, $value = '', $tag = array())
    {
        ...
        $attributes = array(
            ...
            'data-primaryarray' => $this->primary_val_multi,
            'title' => $title
        );
   ...
   protected function create_switch($name, $value = '', $tag = array())
{
        ...
        if ($this->buttons)
        {
            foreach ($this->buttons as $button)
            {
                if ($this->is_button($button['name'], $row))
                {
                    ...
                    if ($button['form-params'])
                    {
                        foreach ($button['form-params'] as $pkey => $pval)
                        {
                            $button['form-params'][$pkey] = $this->replace_text_variables($pval, $row, true);
                        }
                    }
                    ...
                    $tag = array(
                        ...
                        'href' => $link,
                        'title' => $button['name']);
                    $out .= $this->open_tag($tag, $button['class'], $button['params'],$button['form-params']);
                    if ($button['icon'])
                    {
    ...
    public static function load_css()
    {
        ...
        if (Xcrud_config::$load_select2)
            $out .= '<link href="' . Xcrud_config::$scripts_url . '/' . Xcrud_config::$plugins_uri .
                '/select2-develop/dist/css/select2.min.css" rel="stylesheet" type="text/css" />';
    ...
    public static function load_js($current_instance = null)
    {
        $out = '';
        if (self::$instance)
        {
        ...
        if (Xcrud_config::$load_select2)
            $out .= '<script src="' . Xcrud_config::$scripts_url . '/' . Xcrud_config::$plugins_uri .
                '/select2-develop/dist/js/select2.js"></script>';
        if (Xcrud_config::$load_jquery_ui)
            $out .= '<script src="' . Xcrud_config::$scripts_url . '/' . Xcrud_config::$plugins_uri .
                '/jquery-ui-1.12.1/jquery-ui.min.js"></script>';

        if (Xcrud_config::$load_jquery_dirty)
            $out .= '<script src="' . Xcrud_config::$scripts_url . '/' . Xcrud_config::$plugins_uri .
                '/jquery.dirty-develop/dist/jquery.dirty.js"></script>';
        ...
        $out .= '<script src="' . Xcrud_config::$scripts_url . '/' . Xcrud_config::$plugins_uri . '/xcrud.js?v=1.7.25.2"></script>';
        if (Xcrud_config::$load_videojs)
        {
            $out .= '<script src="' . Xcrud_config::$scripts_url . '/' . Xcrud_config::$plugins_uri .
        ...
        $routine_time_in_seconds = isset($current_instance->routine_time_in_seconds) ? $current_instance->routine_time_in_seconds : '';
        $action_event = isset($current_instance->action_event) ? $current_instance->action_event : '';
        $rotation_angle = isset($current_instance->rotation_angle) ? $current_instance->rotation_angle : '';
        $column_visibility_control = isset($current_instance->column_visibility_control) ? $current_instance->column_visibility_control : '';
        ...
        $config = array(
            ...
            'routine_time_in_seconds' => $routine_time_in_seconds,
            'action_event' => $action_event,
            'rotation_angle' => $rotation_angle,
            'column_visibility_control' => $column_visibility_control,
 ...
 'div', $tabs_head = 'ul', $tabs_row = 'li', $tabs_link = 'a', $tabs_content = 'div', $tabs_pane = 'div')
{
            ...
            '&uarr; ', 'desc' => '&darr; '))
    {
            ...
            grid_restrictions) && $this->task != 'print' && $this->buttons_position == 'left')
        {
            ...
            $grid_action_label = "";
            if ($this->theme_config('grid_action_label'))
            {
                $grid_action_label  = $this->theme_config('grid_action_label');
            }
            $out .= $this->open_tag($item, 'xcrud-actions') . '<center>'  . $grid_action_label . '</center>' . $this->close_tag($item);
        }
        ...
        if($this->is_bulk_select || $this->grid_restrictions){                  
         ...
         /*print_r($this->column_rotation); 
         print_r($this->rotation_angle);
         print_r($this->column_switch);
         print_r($this->column_fix);
         print_r($this->column_resize);*/
         $columns_resize = $this->column_resize;
         $columns_resizeArray = explode(',', $columns_resize);

         $column_rotation = $this->column_rotation;
         $column_rotationArray = explode(',', $column_rotation);



         //print_r($columns_resizeArray);
        foreach ($this->columns as $field => $fitem)
        {
            
            $class_resize = "";
            $class_rotation_angle = "";

            $field_def = $fitem["field"];

            if (in_array($field_def, $columns_resizeArray)) {
                $class_resize = "xcrud-resize";
            } else {
                //$class_resize = "xcrud-resize";
            }

            if (in_array($field_def, $column_rotationArray)) {
                $class_rotation_angle = "xcrud-rotation";
            } else {
                //$class_resize = "xcrud-resize";
            }
            ...
            if (isset($this->field_type[$field]) && ($this->field_type[$field] == 'password' or $this->field_type[$field] ==
                ...
                continue;
            $fieldname = $this->columns_names[$field];
            $class = 'xcrud-column ' . $class_resize . " " . $class_rotation_angle;
            $attr = array();
            if ($this->is_sortable)
            {
                $class .= ' xcrud-action';
    ...
    protected function render_button($name = '', $task = '', $after = '', $class = '', $icon = '', $mode = '', $primary = '')
    {
        ...
        if (!isset($this->hide_button[$name]))
        {
            ...
            if ($this->primary_val_multi)
            {
                $tag['data-primaryarray'] = $this->primary_val_multi;
            }
