...
var Xcrud = {
    ...
    init_actions: function(container) {
        ...
        if ($(".xcrud").length) {
            ...
            $(".xcrud").on("click", ".xcrud-action", function() {
                ...
                var confirm_form = $(this).data('form');
                
                if (confirm_text && !window.confirm(confirm_text)) {
                    return;
                }else if (confirm_form) {
                    //pop form and submit
                    //continue with code
                    var buttonHTML = $(this).prop('outerHTML');
                    $("#" + confirm_form).append(buttonHTML);
                    return;
            ...
            $(".xcrud-ajax").each(function() {
                ...
                Xcrud.formular_logic_init(this);
    ...
    isValidFormula: function(formula) {
        // This regex allows digits, spaces, and basic arithmetic operators (+, -, *, /, (), .)
        const validFormulaRegex = /^[0-9+\-*/().\s]+$/;

        // Test the formula against the regex
        return validFormulaRegex.test(formula);
    },
    formular_logic_init: function(container) {
          // Get the formular-identifier from the triggering element
          let values = {};
          $(container).find('.xcrud-input[formular-identifier]').each(function() {

              var val_var = $(this).attr("formular-identifier");
              var val_event = $(this).attr("formular-event");
              var val_name = $(this).attr("formuar-name");
              var event_application = "";

              if (val_event == "onchange") {
                  event_application = "change";
              } else if (val_event == "onclick") {
                  event_application = "click";
              } else if (val_event == "onmouseover") {
                  event_application = "mouseover";
              } else if (val_event == "onmouseout") {
                  event_application = "mouseout";
              } else if (val_event == "onkeydown") {
                  event_application = "keydown";
              } else if (val_event == "onkeyup") {
                  event_application = "keyup";
              } else if (val_event == "onkeypress") {
                  event_application = "keypress";
              } else if (val_event == "onfocus") {
                  event_application = "focus";
              } else if (val_event == "onblur") {
                  event_application = "blur";
              } else {
                  event_application = "";
              }

              //console.log(">>>" + val_var,event_application);

              $(container).on(event_application, '.xcrud-input[formuar-name="' + val_name + '"]', function() { //On click

                  $(container).find('.xcrud-input[formular-identifier="' + val_var + '"]').each(function(e) {
                      var $currentElement = $(this);
                      const formularName = $currentElement.attr("formuar-name");
                      values[formularName] = $currentElement.val();
                  });

                  $(container).find('.xcrud-input[formular-answer="' + val_var + '"]').each(function(e) {
                      var answerElement = $(this);
                      if (answerElement) {
                          const formula = Xcrud.decode(answerElement.attr('formular'));
                          let resultFormula = formula;

                          // Extract and apply placeholders
                          for (const key in values) {
                              const placeholder = `{${key}}`;
                              const escapedValue = values[key].replace(/[@]/g, '\\$&'); // Escape special characters like @
                              resultFormula = resultFormula.replaceAll(placeholder, escapedValue);
                          }

                          // Extract method if exists (e.g., [toLowerCase()])
                          let methodMatch = resultFormula.match(/\[(\w+)\(\)]$/);
                          let method = null;
                          if (methodMatch) {
                              method = methodMatch[1];
                              resultFormula = resultFormula.replace(/\[(\w+)\(\)]$/, '');
                          }

                          let result = "";
                          if (Xcrud.isValidFormula(resultFormula)) {
                              try {
                                  result = eval(resultFormula);
    ...
    display_logic_init: function(container) {
        ...
        $(container).find('.xcrud-input[data-display-logic-src]').each(function() {
                ...
                $(container).find('.xcrud-input[data-display-logic="' + depend_on_ + '"]').each(function(e) {
                    ...
                    var data_depend_value_ = "";
                      var dependval_ = "";

                      if (Xcrud.isNumeric(data_depend_value) && Xcrud.isNumeric(dependval)) {
                        // Convert string numbers to JavaScript numbers
                        data_depend_value_ = parseInt(data_depend_value);
                        dependval_ = parseInt(dependval);
                      }else{
                        data_depend_value_ = data_depend_value;
                        dependval_ = dependval;
                      }


                      //console.log(id + "clicked.." + data_operator + "..." + data_depend_value_ + "..." + dependval_);
                      if (Xcrud.check_container(this, container)) {
                          ...
                          if(data_operator == "="){
                              if(data_depend_value_ == dependval_){
                                 $display = true;
                              }
                          }

                          if(data_operator == ">"){
                              if(dependval_ > data_depend_value_){
                                 $display = true;
                          ...
                          if(data_operator == "IN"){
                              var arr = data_depend_value_.split(",");
                              if (arr.includes(dependval_)) {
                                  $display = true;
                              } 
                          }
                ...
                $(container).find('.xcrud-input[data-display-logic="' + depend_on_ + '"]').each(function(e) {
                    ...
                    data_depend_value = $currentElement.attr("data-depend-value");
                      ...
                      if (Xcrud.check_container(this, container)) {
                          ...
                          if(data_operator == "IN"){
                              var arr = data_depend_value.split(",");
                              if (arr.includes(dependval)) {
                                  $display = true;
                              } 
                          }
        ...
        $(container).find('.xcrud-input[data-readonly-logic-src]').each(function() {
                ...
                $(container).find('.xcrud-input[data-readonly-logic="' + depend_on_ + '"]').each(function(e) {
                    ...
                    */
                     ...
                     var data_depend_value_ = parseInt(data_depend_value);
                      var dependval_ = parseInt(dependval);
                      if (Xcrud.check_container(this, container)) {
    ...
    depend_init: function(container) {
        ...
        let routine_time_in_seconds = "";
        if (Xcrud.config('routine_time_in_seconds')) {
           routine_time_in_seconds = Xcrud.config('routine_time_in_seconds');
        }

        let action_event = "";
        if (Xcrud.config('action_event')) {
           action_event = Xcrud.config('action_event');
        }

        let rotation_angle = "";
        if (Xcrud.config('rotation_angle')) {
           rotation_angle = Xcrud.config('rotation_angle');
        }

        let column_visibility_control = "";
        if (Xcrud.config('column_visibility_control')) {
           column_visibility_control = Xcrud.config('column_visibility_control');
        }

     

        var actions = action_event.split(",");
        for (var action of actions) {
  
                if (action === 'autorefresh') {
                    const intervalTime = parseInt(routine_time_in_seconds, 10) * 1000;
                    setInterval(function() {
                        //Xcrud.reload(container,false);
                        var inputElement = $(container).find('input[name="task"]');

                        //var inputElement = document.querySelector('input[name="task"][value="list"]');
         

                        // Check if the input element exists and has the value "list"
                        if ($(inputElement).val() == "list" || $(inputElement).val() == "refresh") {
                            Xcrud.request('.xcrud-ajax',Xcrud.list_data('.xcrud-ajax',{action:'refresh', task:'refresh',after:'list'}));
                        } else {
                            console.log("The input element does not have the value 'list'.");
                        }
                    }, intervalTime);

                }else if (action === 'columnresize') {
                      const $container = $(container);
                      const $ths = $container.find('th.xcrud-resize');
                      ...
                      $ths.each(function() {
                          ...
                          $resizer.on('mousedown', function(e) {
                              ...
                              e.stopPropagation(); // Stop propagation to avoid sorting
                      ...
                      function initResize(e) {
                          const $th = $(e.target).parent();
                          const startX = e.pageX;
                          const startWidth = $th.outerWidth();
                          const $nextTh = $th.next();

                          if ($nextTh.length === 0) return; // If there's no next column, exit

                          const startNextWidth = $nextTh.outerWidth();
                          ...
                          function doResize(e) {
                              ...
                              if (newWidth > 30 && newNextWidth > 30) { // Minimum width constraints
                          ...
                          function stopResize() {
                              ...
                              setTimeout(() => { isResizing = false; }, 0); // Allow the flag to be reset after all events are handled
                          }
                      }

                      $ths.on('click', function(e) {
                          if (isResizing) {
                              e.preventDefault(); // Prevent sorting if resizing
                              e.stopPropagation(); // Stop propagation to avoid sorting
                          }
                      });
                  }
                  ...
                  else if(action === 'columnrotation') {

                      //const rotation_angle = 45; // Example angle, you can change this as needed

                      const $container = $(container);
                      const $ths = $container.find('th.xcrud-rotation');

                      console.log("Found headers to rotate:", $ths);
                      $ths.each(function() {
                          const $th = $(this);
                          const content = $th.html();
        
                          // Wrapping content in a span for better positioning
                          $th.html('<span class="rotated-content">' + content + '</span>');
        
                          const $rotatedContent = $th.find('.rotated-content');
                          $rotatedContent.css({
                              'display': 'inline-block',
                              'transform': 'rotate(' + rotation_angle + 'deg)',
                              'transform-origin': 'bottom right', // Adjust to 'bottom left' for correct alignment
                              'white-space': 'nowrap',
                              'position': 'absolute',
                              'bottom': '0',
                              'left': '0',
                          ...
                          // Ensure the th itself has relative positioning
                          $th.css('position', 'relative');

                          // Calculate the required height to accommodate the rotated content
                          const originalWidth = $rotatedContent.outerWidth();
                          const originalHeight = $rotatedContent.outerHeight();
        
                          const rotatedHeight = Math.abs(originalWidth * Math.sin(rotation_angle * Math.PI / 180)) + 
                                                Math.abs(originalHeight * Math.cos(rotation_angle * Math.PI / 180));

                          // Set the height of the th element to accommodate the rotated content
                          $th.css('height', rotatedHeight + 'px');

                          // Adjust the left position of the rotated content for perfect alignment
                          const rotatedWidth = $rotatedContent[0].getBoundingClientRect().width;
                          //$rotatedContent.css('left', -(rotatedWidth * Math.sin(rotation_angle * Math.PI / 180) / 2) + 'px');
                          $rotatedContent.css('left', -(rotatedHeight * Math.sin(rotation_angle * Math.PI / 180) / 2) + 'px');
                          // Move resizer out of the rotated content
                          const $resizer = $th.find('.resizer');
                          $th.append($resizer);
                      });
                  }else if (action === 'column_visibility_control'){
                     
                       
                      if(column_visibility_control){

                            const $container = $(container);
                            const useSelect2 = true; // Toggle this to false to use normal multi-select
                            const $table = $container.find('.xcrud-list');
                            const columns = [];
                            const localStorageKey = 'columnVisibility';

                            // Create select box and label dynamically
                            const $div = $('<div class="btn-group pull-right"></div>');  //.prependTo($container.find(".xcrud-top-actions"));
                            
                            const $label = $('<label for="columnSelector">Select Columns: </label>').appendTo($div);
                            const $columnSelector = $('<select id="columnSelector" multiple="multiple" style="width: 300px;"></select>').appendTo($div);
                            $container.find(".xcrud-top-actions.list-only").append($div);

                            // Initialize columnSelector with column names
                            $table.find('th.xcrud-column,th.xcrud-actions').each(function(index) {
                                const $th = $(this);
                                const columnName = $th.text();
                                columns.push({ id: index, text: columnName });
                                $th.data('index', index);
                            });

                            // Populate the select options
                            columns.forEach(column => {
                                $columnSelector.append(`<option value="${column.id}">${column.text}</option>`);
                            });
                            ...
                            const toggleColumnVisibility = (selectedIndices) => {
                                // Show/hide headers based on selection
                                $table.find('th.xcrud-column,th.xcrud-actions').each(function() {
                                    const $th = $(this);
                                    const columnIndex = $th.data('index');
                                    console.log("d",columnIndex);
                                    console.log("e",selectedIndices);
                                    if (selectedIndices.includes(columnIndex)) {
                            ...
                            // Load saved column visibility state from local storage
                            const savedState = JSON.parse(localStorage.getItem(localStorageKey));

                            if (savedState) {
                                $columnSelector.val(savedState).trigger('change');
                                toggleColumnVisibility(savedState);
                            } else {
                                const defaultState = columns.map(col => col.id.toString());
                                $columnSelector.val(defaultState).trigger('change');
                                toggleColumnVisibility(defaultState.map(Number));
                            }

                            // Handle column visibility toggle
                            $columnSelector.on('change', function() {
                                const selectedIndices = $(this).val().map(Number);
                                toggleColumnVisibility(selectedIndices);

                                // Save the selected indices to local storage
                                localStorage.setItem(localStorageKey, JSON.stringify(selectedIndices));
                            });
                      }
    ...
    semantic_modal: function(header, content, e = "") {
        ...
        Xcrud.formular_logic_init($("#xcrud-modal-window"));
            ...
            if (!isActionsCell) {
                ...
                Xcrud.request_inline_edit('.xcrud-ajax', Xcrud.list_data('.xcrud-ajax', { key: instanceKey, instance: instanceVal, primary: primaryKey, task: 'edit', editmode: 'inline', field: order_by }), e, function(response) {
                    ...
                    if (response != "") {
                        ...
                        Xcrud.formular_logic_init(clickedCell);
    ...
    deleteItems: function(button) {
        ...
        if (r == true) {
            // Find the closest '.xcrud-ajax' element to the button
            var closestXcrud = $(button).closest('.xcrud-ajax');
        
            // Check if a closest '.xcrud-ajax' element was found
            if (closestXcrud.length > 0) {
                Xcrud.request(closestXcrud, Xcrud.list_data(closestXcrud, { after:'list', task: 'remove', primary: items }), function () {
                    alert("Items deleted");
                ...
                alert("No '.xcrud-ajax' element found.");
            }
    ...
    editItems: function(button) {
        var r = confirm("Confirm edit of " + items.length + " items.");
        if (r == true) {
            // Find the closest '.xcrud-ajax' element to the button
            var closestXcrud = $(button).closest('.xcrud-ajax');
            ...
            if (closestXcrud.length > 0) {
                ...
                let resultString = items.join(',');
                Xcrud.request(closestXcrud, Xcrud.list_data(closestXcrud, { task: 'edit',primaryarray:resultString, primary: items[0] }), function () {
                    //alert("Items deleted");
                });
            } else {
                alert("No '.xcrud-ajax' element found.");
    ...
    processAttributes: function (htmlString) {
        ...
        for (var i = 0; i < element.attributes.length; i++) {
            ...
            var name = attr.name.replace('data-', ''); // Remove 'data-' prefix
            attributes[name] = attr.value;
        }
        return attributes;
    },
    generateForm: function(fieldsAll) {
        fields = fieldsAll[0];
        let html = '<div id="form1" class="form" style="top: 0%;display:block;width: 40%;position: absolute;background: #fff;left: 25%;z-index: 1000;border: 1px solid;padding: 15px;">';
        ...
        fields.forEach(field => {
            ...
            if (field.type === 'select') {
                html += '<select class="form-control" id="' + field.id + '" name="' + field.id + '">';
                html += field.options;
                html += '</select>';
            } else if (field.type === 'textarea') {
                html += `<textarea class="form-control" id="${field.id}" name="${field.id}" rows="3"></textarea>`;
            } else {
        ...
        html += '<a style="margin:5px;" class="submit_button_link" href="javascript:void(0);">submit</a>';
        html += '<a style="margin:5px;" class="exit_button_link" href="javascript:void(0);">exit</a>'; // Add exit button
        html += '</div>';
...
$(document).ready(function() {
    ...
    if ($(".xcrud").length) {
        ...
        $(".xcrud").on("click", ".xcrud-action", function() {
            ...
            var confirm_form = $(this).data('form');
            var confirm_form_id = $(this).data('form-id');
            var confirm_form_json = $(this).data('form-json');
            var confirm_text = $(this).data('confirm');
            var confirm_form_logic = $(this).data('form-confirm-logic');
            var confirm_form_logic_failure_message = $(this).data('form-confirm-logic-failure-message');
            //data-form-accept
            ...
            if (confirm_text && !window.confirm(confirm_text)) {
                ...
                if(confirm_form_id != ""){
                    $("#" + confirm_form).show();
                    var buttonHTML = $(this).prop('outerHTML');
                       var processedAttributes = Xcrud.processAttributes(buttonHTML);

                      // Modify attribute names and pass them into the function
                      var modifiedAttributes = {};
                      for (var key in processedAttributes) {
                          if (processedAttributes.hasOwnProperty(key)) {
                              key = key.replace('data-', '');
                              modifiedAttributes[key] = processedAttributes[key];
                          }
                      }

                      $("#" + confirm_form + " .submit_button").html("");
                      $("#" + confirm_form + " .submit_button").append("<a class='submit_button_link' href='javascript:void(0);'>submit</a>");

                      $("#" + confirm_form).off("click", ".submit_button_link");
                      $("#" + confirm_form).on("click", ".submit_button_link", function(event) {
                          var formElements = $("#" + confirm_form).find("input, textarea, select");
                          // Loop through each form element
                          formElements.each(function() {
                              // Get the ID of the current form element
                              var elementId = $(this).attr('id');

                              // Get the value of the current form element
                              var elementValue = $(this).val();

                              // Add ID and value to modifiedAttributes
                              modifiedAttributes[elementId] = elementValue;
                          });
                          event.preventDefault(); // Prevent default link behavior
                          Xcrud.request('.xcrud-ajax', Xcrud.list_data('.xcrud-ajax', modifiedAttributes),function(){
                ...
                }else if(confirm_form_json != ""){

                   var decodedString = decodeURIComponent(confirm_form_json);
                   const fields = decodedString.split(/\n+/).map(entry => {
                      // Remove unnecessary characters and replace '+' with ' '
                      entry = entry.replace(/\++/g, ' ').replace(/"(\w+)":\s(\+)"(\w+)"(?:,|$)/g, '"$1": "$3"');

                      // Handle options property separately
                      entry = entry.replace(/"options":\+"([^"]+)"(?:,|$)/g, '"options": "$1"');
                     
                      // Parse the cleaned entry as JSON
                      return JSON.parse(entry);
                  });

                  htmlForm = Xcrud.generateForm(fields);
                  ...
                  // Append the button HTML to the form HTML
                 // Append the button HTML to the form HTML
                  //htmlForm += "<a class='submit_button_link' href='javascript:void(0);'>submit</a>";
                  ...
                  $(".xcrud-list-container").off("click", "#" + confirm_form + " .submit_button_link").on("click", "#" + confirm_form + " .submit_button_link", function(event) {

                      var formElements = $("#" + confirm_form).find("input, textarea, select");
                    
                      // Loop through each form element
                      var updated_confirm_form_logic = "";
                      formElements.each(function() {
                          // Get the ID of the current form element
                          var elementId = $(this).attr('id');
                          ...
                          if (confirm_form_logic.includes(elementId)) {
                              updated_confirm_form_logic = confirm_form_logic.replace(new RegExp(elementId, 'g'), elementValue);
                          }
                      ...
                      if(eval(updated_confirm_form_logic)){
                            event.preventDefault(); // Prevent default link behavior

                            Xcrud.request('.xcrud-ajax', Xcrud.list_data('.xcrud-ajax', modifiedAttributes), function() {
                                $("#" + confirm_form).off().remove();
                                //Xcrud.reload();
                            });
                      } else{
                            alert(confirm_form_logic_failure_message);
                            $("#" + confirm_form).off().remove();
                            //Xcrud.reload();
                      }  
                  });

                  // Check if a form with ID form1 exists before appending
                  if ($('#form1').length > 0) {
                        $("#form1").off().remove();
                  }

                  
                  $(".xcrud-list-container").append(htmlForm);

                  // Attach event handler for exit button
                  $(".xcrud-list-container").off("click", ".exit_button_link").on("click", ".exit_button_link", function(event) {
                  // Destroy the whole form rather than hide
                        $("#form1").off().remove();
                  });
...
$(document).on("xcrudafterrequest", function(event, container) {
    ...
    Xcrud.formular_logic_init(container);
    Xcrud.disable_logic_init(container);
    Xcrud.readonly_logic_init(container);
